...
function Form({ queries, data, config, onReady, enums }) {
  const classes = useStyles();
  const [formData, setFormData] = useState(
    data ||
      _.fromPairs(
        _.reject(config, ['editable', false]).map(item => [
          item.key,
  ...
  const dictionaries = queries.GET_DICTIONARIES_QUERY ? useQuery(queries.GET_DICTIONARIES_QUERY) : {};
...
Form.propTypes = {
  ...
  config: PropTypes.arrayOf(headerShape).isRequired,
